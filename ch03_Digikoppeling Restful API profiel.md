...
| Verplicht | API-58  No sensitive information in URIs | Security | Vermijd query strings met gevoelige informatie in bevragingen via _HTTP method_ `GET`. (**) | [API-58 No sensitive information in URIs  ](https://docs.geostandaarden.nl/api/def-hr-API-Strategie-ext-20211013/#api-58)|
...
(**) Voor deze gevoelige informatie dient gebruik te worden gemaakt van de _HTTP method_ `POST`. Een endpoint dient beschikbaar gesteld te worden om bevragingen via _HTTP method_ `POST` te ondersteunen. Gevoelige query parameters worden in de _payload_ geplaatst als _content type_ `application/json`. Bijvoorbeeld, de query  `?field1=value1&field2=value2` wordt via POST overgebracht als:
```json
{
  "field1": "value1",
  "field2": "value2"
